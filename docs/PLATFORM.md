...
| **Railway** `open_claw` | 2 service 24/7: bot + gateway | Database chính |
...
|---------|----------------|--------|
| `telegram-bot` | `apps/telegram-bot` | Không (chỉ Telegram API) |
| `openclaw-gateway` | `apps/gateway` | Không (private network) |

Mỗi service trên Railway: **Settings → Root Directory** = cột trên; `railway.toml` nằm trong thư mục root đó.

---

## 3. Railway — hai service

### 3.1 `telegram-bot` (cơ bắp vận hành)
...
Railway private networking: hai service cùng project `open_claw` gọi nhau qua hostname service.
...
| `deploy-railway.yml` | push `main` | Deploy 2 service (Railway token secret) |
...
| Railway `open_claw` | User đã tạo project — cần 2 service + env |
